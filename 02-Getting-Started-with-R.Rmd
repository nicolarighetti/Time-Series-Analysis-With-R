...
*   employ specific libraries such as **tidyverse**. Tidyverse is a collection of libraries and functions for data science [https://www.tidyverse.org](https://www.tidyverse.org). 

In R, a **library (or "package")** is a coherent collection of functions, usually created for a specific purposes.

To work with the *tidyverse* library, it is necessary to install it first using the following command: *install.packages("tidyverse")*.

### Objects 

An *object* is an R entity composed of a name and a value.

The **arrow <-** sign is used to *create objects* and *assign a value* to an object (or to change ("update") its previous value).
...
The object is equal to its value. Therefore, for instance, an object with a numerical value can be used to perform arithmetical operation.
...
Tidyverse is an R library composed of functions that allow users to perform basic and advanced data science operations. 

Tidyverse has a peculiar syntax that includes the so-called [**pipe**](https://style.tidyverse.org/pipes.html) operator: **%>%**.

To manipulate data sets are useful the functions included in [**dplyr**](https://dplyr.tidyverse.org): *a grammar of data manipulation, providing a consistent set of verbs that help you solve the most common data manipulation challenges*.

Since we have already installed tidyverse, we have to load it, so as we can work with its functions in the current R session:
...
Variable can have many different *formats*, such as:

*   **double** is for numbers that includes decimals, and is therefore more appropriate to *continuous variables*;
*   **integer** is suitable for count data or  *discrete variables*;
*   **factors** are used for categorical varibales. Factors can be ordered (e.g.: level of agreement: "high", "medium", "low") or not (e.g.: hair colors "blond", "dark brown", "brown");
*   **characters** for textual labels;
*   **logicals** for logical values (i.e.: TRUE and FALSE)
*   **dates** to represent days;
*   **POSIX**, a particular R format to represent date and times.

To check the data format of the variables stored in the data.frame can be used the following command ("num" means "numeric"):

```{r}
str(new_dataframe)
```

Sometimes variables are stored in the data.frame in the wrong format and we want to convert them into a new data format. For this purpose we can use, again, the function **mutate**, along with other functions such *as.integer*, *as.numeric*, *as.character*, *as.factors*, or *as.logical*, *as.Date*, or *as.POSIXct()* based on the desired data format.

```{r fig.cap="R data formats. Tables from Gaubatz, K. T. (2014). [A Survivor's Guide to R: An Introduction for the Uninitiated and the Unnerved](https://us.sagepub.com/en-us/nam/a-survivors-guide-to-r/book242607). SAGE Publications.[Google Trends](https://trends.google.com)", echo=FALSE}
